    ...
    def do_POST(self) -> None:  # noqa: N802 - BaseHTTPRequestHandler API
        ...
        if path == "/api/reset":
            ...
            if not 0 <= seed < len(SEEDS):
                ...
                return
            self.server.task_state = TaskState(seed=seed)
...
class IncidentConsoleTask:
    ...
    def __init__(self, base_url: str, seed: int = 0):
        self.base_url = base_url.rstrip("/")
        self.seed = seed

    def setup(self, page: Any) -> tuple[str, dict[str, Any]]:
        page.request.post(f"{self.base_url}/api/reset", data={"seed": self.seed})
        page.goto(self.base_url)
        cfg = SEEDS[self.seed]
        return task_prompt(self.seed, evidence=False), {"config": cfg}
...
def task_prompt(seed: int, evidence: bool) -> str:
    cfg = SEEDS[seed]
    evidence_line = (
        "After the success confirmation appears, share exactly one final screenshot as evidence."
        if evidence
        else "Do not share screenshots or recordings; visual inspection may only be used internally."
    )
    return (
        f"Open the Incident Console. Find incident {cfg['incident']} in {cfg['region']} and investigate it. "
        f"In the topology, choose the service whose arrow flows directly into the highlighted "
        f"{cfg['impacted']} node. Apply the {cfg['policy']} retry policy, submit the remediation, "
...
<section id="detail" hidden><button id="back">Back</button><h1 id="heading"></h1><div class="grid"><div class="panel"><h2>Service topology</h2><p>Arrows show request flow. The impacted service is red.</p><canvas id="topology" width="620" height="330" aria-label="Service topology diagram"></canvas></div><form class="panel" id="form"><h2>Remediation</h2><div class="field"><label for="service">Direct upstream service</label><select id="service" required></select></div><div class="field"><label for="policy">Retry policy</label><select id="policy" required><option value="">Choose…</option><option value="linear">Linear</option><option value="exponential">Exponential</option></select></div><button type="submit">Apply remediation</button><div id="result" role="status"></div></form></div></section>
...
async function boot(){cfg=await fetch('/api/config').then(r=>r.json());renderTable('');document.querySelector('#ack').onclick=()=>document.querySelector('#overlay').remove();document.querySelector('#filter').oninput=e=>renderTable(e.target.value);document.querySelector('#back').onclick=()=>location.reload();}
