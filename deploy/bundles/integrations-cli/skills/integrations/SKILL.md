---
name: integrations
description: Third-party integration CLIs (e.g. gh for GitHub, pup for Datadog, slack for Slack) are available in this session, authenticated automatically by the engrams platform. Use when you need to interact with a connected service from the command line. Run `engrams-integrations` to see which are enabled and how to use each.
---

# Integration CLIs

This session has command-line tools for the third-party services your profile
connected — for example `gh` (GitHub), `pup` (Datadog), and `slack` (Slack). **You never
