...
const MAX_SOURCE_PREVIEW_CHARS = 20_000;
...
export function SpecBlockTranscriptChip({
  ...
  const sourcePreview = source?.slice(0, MAX_SOURCE_PREVIEW_CHARS) ?? null;
  const sourceTruncated = source !== null && source.length > MAX_SOURCE_PREVIEW_CHARS;
      ...
      {sourcePreview ? (
        <details className="mt-2 border-t pt-2 text-xs">
          <summary className="flex cursor-pointer list-none items-center gap-1 text-muted-foreground">
            ...
            Source change{sourceTruncated ? " (preview)" : ""}
          </summary>
          <pre className="mt-2 max-h-48 overflow-auto whitespace-pre-wrap text-foreground">
            {sourcePreview}
          </pre>
          {sourceTruncated ? (
            <div className="mt-1 text-muted-foreground">The source preview is limited.</div>
          ) : null}
        </details>
      ) : null}
    </div>
