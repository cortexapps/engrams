...
describe('buildMessages — message/part shaping', () => {
  test('a user-role agent_message becomes a user message with one text part', () => {
    const { messages } = buildMessages(
      indexed([
        { type: 'agent_message', run_id: '', message_id: 'u1', role: 'user', text: 'fix the flaky test', at: AT },
      ]),
      SID,
    );
    const msgs = real(messages);
    expect(msgs).toHaveLength(1);
    expect(msgs[0]).toMatchObject({ role: 'user', content: [{ type: 'text', text: 'fix the flaky test' }] });
  });
  ...
  test("a run's assistant text + tool calls collapse into one assistant message, parts in order", () => {
    const { messages } = buildMessages(
      indexed([
        ...
        { type: 'agent_message', run_id: 'r1', message_id: 'a1', role: 'assistant', text: 'looking', at: AT },
        { type: 'tool_call_started', run_id: 'r1', tool_call_id: 't1', tool_name: 'Read', args_summary: '{"file_path":"a.rs"}', at: AT },
        { type: 'tool_call_completed', run_id: 'r1', tool_call_id: 't1', tool_name: 'Read', ok: true, duration_ms: 11, result_summary: '212 lines', at: AT },
  ...
  test('consecutive assistant messages coalesce into one prose text part', () => {
    const { messages } = buildMessages(
      indexed([
        { type: 'run_started', run_id: 'r1', prompt_summary: null, at: AT },
        { type: 'agent_message', run_id: 'r1', message_id: 'a1', role: 'assistant', text: 'one', at: AT },
        { type: 'agent_message', run_id: 'r1', message_id: 'a2', role: 'assistant', text: 'two', at: AT },
        { type: 'run_completed', run_id: 'r1', ok: true, at: AT2 },
      ]),
      SID,
    );
  ...
  test('a shell exec becomes a synthetic engram.shell tool part, output streamed into result', () => {
    const { messages } = buildMessages(
      indexed([
        ...
        { type: 'exec_completed', exec_id: 'x1', exit_status: 0, rusage: { duration_ms: 4200 }, at: AT2 },
        { type: 'run_completed', run_id: 'r1', ok: true, at: AT2 },
      ]),
      SID,
    );
  ...
  test('a nonzero exit marks the shell part as an error', () => {
    const { messages } = buildMessages(
      indexed([
        ...
        { type: 'exec_completed', exec_id: 'x1', exit_status: 1, rusage: { duration_ms: 3 }, at: AT2 },
  ...
  test('a completed run carries the read/edit/ran tally as a footer', () => {
    const { messages } = buildMessages(
      indexed([
        { type: 'run_started', run_id: 'r1', prompt_summary: null, at: AT },
        { type: 'tool_call_started', run_id: 'r1', tool_call_id: 't1', tool_name: 'Read', args_summary: '{"file_path":"a.rs"}', at: AT },
        { type: 'tool_call_started', run_id: 'r1', tool_call_id: 't2', tool_name: 'Edit', args_summary: '{"file_path":"a.rs"}', at: AT },
    ...
    expect(footer).toMatchObject({ reads: 1, edits: 1, ran: 1, other: 0, ok: true, interrupted: false });
  });
  ...
  test('snapshot / resumed become durability system markers', () => {
    ...
    expect(customMarker(msgs[0]!)).toMatchObject({ kind: 'durability', mark: 'snapshot', sizeBytes: 1_287_000_000 });
  ...
  test('pull_request_opened becomes a pull_request system marker', () => {
    const { messages } = buildMessages(
      indexed([
        {
          type: 'pull_request_opened', url: 'https://gh/x/pull/7', repo: 'x/y', title: 'Fix it',
          number: 7, head_branch: 'fix', base_branch: 'main', at: AT,
        },
      ]),
      SID,
    );
    expect(customMarker(real(messages)[0]!)).toMatchObject({ kind: 'pull_request', number: 7, repo: 'x/y' });
  });

  test('file_shared becomes an artifact system marker carrying the session id', () => {
    const { messages } = buildMessages(
      indexed([
        { type: 'file_shared', artifact_id: 'art1', media_type: 'image/png', size_bytes: 9000, caption: 'a shot', at: AT },
      ]),
      SID,
    );
    expect(customMarker(real(messages)[0]!)).toMatchObject({
      kind: 'artifact', sessionId: SID, artifactId: 'art1', mediaType: 'image/png', caption: 'a shot',
    });
  });
  ...
  test('a system-role agent_message becomes a note system marker', () => {
    const { messages } = buildMessages(
      indexed([
        { type: 'agent_message', run_id: 'r1', message_id: 's1', role: 'system', text: 'context compacted', at: AT },
      ]),
      SID,
    );
...
describe('buildMessages — isRunning', () => {
  test('an open tool call (no completion) is running', () => {
    const { isRunning } = buildMessages(
      indexed([
        { type: 'run_started', run_id: 'r1', prompt_summary: null, at: AT },
        { type: 'tool_call_started', run_id: 'r1', tool_call_id: 't1', tool_name: 'Read', args_summary: null, at: AT },
      ]),
      SID,
    );
  ...
  test('a completed run ending in an assistant message is not running', () => {
    const { isRunning } = buildMessages(
      indexed([
        { type: 'run_started', run_id: 'r1', prompt_summary: null, at: AT },
        { type: 'agent_message', run_id: 'r1', message_id: 'a1', role: 'assistant', text: 'done', at: AT },
        { type: 'run_completed', run_id: 'r1', ok: true, at: AT2 },
      ]),
      SID,
    );
