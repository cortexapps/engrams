...
export const saveClaudeToken = (token: string) => postJSON<void>("/me/claude-token", { token });

export const deleteClaudeToken = () => deleteEmpty("/me/claude-token");
...
export const fetchSessions = (scope?: "mine" | "all") =>
  getJSON<ListSessionsResponse>(`/sessions${scope ? `?scope=${scope}` : ""}`).then(
    (r) => r.sessions,
  );

export const fetchHosts = () => getJSON<ListHostsResponse>("/hosts").then((r) => r.hosts);
...
export const drainHost = (hostId: string) => postJSON<void>(`/hosts/${hostId}/drain`, {});

export const fetchSession = (id: string) => getJSON<Session>(`/sessions/${id}`);
...
export const fetchStorageSummary = () => getJSON<StorageSummaryResponse>("/storage/summary");
...
export const sendPrompt = (sessionId: string, text: string) =>
  postJSON<{ session_id: string; note: string }>(`/sessions/${sessionId}/prompt`, { text });
...
export const fetchEnabledImages = () =>
  getJSON<ListEnabledImagesResponse>("/enabled-images").then((r) => r.images);
