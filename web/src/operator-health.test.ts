...
const row = (last_flush_at: string | null): DurabilityRow => ({
  ...
  base_chunks: 0, base_chunks_local: 0, last_flush_at,
});
const storage = (p: Partial<StorageSummaryResponse>): StorageSummaryResponse => ({
  snapshots: 0, snapshot_bytes: 0, gc_pending: 0, tracked_sandboxes: 0,
  dirty_chunks: 0, unflushed_bytes: 0, avg_locality_pct: 0, rows: [], ...p,
});
...
describe('deriveHealthMetrics', () => {
  ...
  test('locality stays null until something is chunk-tracked', () => {
    expect(deriveHealthMetrics([host('ready')], storage({ tracked_sandboxes: 0, avg_locality_pct: 90 })).locality).toBeNull();
    expect(deriveHealthMetrics([host('ready')], storage({ tracked_sandboxes: 3, avg_locality_pct: 90 })).locality).toBe(90);
  });

  test('only rows past the flush window count as RPO-stale', () => {
    const m = deriveHealthMetrics([host('ready')], storage({
      tracked_sandboxes: 3, rows: [row(STALE), row(fresh()), row(null)],
    }));
    expect(m.rpoStale).toBe(2); // the stale one and the never-flushed null
  });

  test('host states are tallied', () => {
    const m = deriveHealthMetrics([host('ready'), host('draining'), host('dead'), host('dead')], undefined);
...
describe('operatorIssues', () => {
  ...
  test('a dead host is critical and named with a count', () => {
    expect(operatorIssues({ ...base, dead: 2 })).toEqual([{ tone: 'critical', text: '2 hosts offline' }]);
  });

  test('capacity crosses caution at 70 and critical at 90, never both', () => {
    expect(operatorIssues({ ...base, capPct: 75 })).toEqual([{ tone: 'caution', text: 'fleet at 75% capacity' }]);
    expect(operatorIssues({ ...base, capPct: 95 })).toEqual([{ tone: 'critical', text: 'fleet at 95% capacity' }]);
  });
  ...
  test('worst issue sorts first regardless of input order', () => {
    ...
    expect(issues.every((i, x) => x === 0 || i.tone !== 'critical' || issues[x - 1].tone === 'critical')).toBe(true);
  });
});
