...
const CAPACITY_ZONES: Zone[] = [{ to: 70, tone: 'nominal' }, { to: 90, tone: 'caution' }, { to: 100, tone: 'critical' }];
// Locality reads the other way: high is good, so the bands invert.
const LOCALITY_ZONES: Zone[] = [{ to: 50, tone: 'critical' }, { to: 80, tone: 'caution' }, { to: 100, tone: 'nominal' }];

interface Verdict { tone: Tone | 'neutral'; headline: string; more: number }
...
export function Overview() {
  ...
  return (
    <div className="space-y-8">
      <PageHeading title="Operator" description="Fleet capacity and storage durability at a glance." />
      ...
      <section className="rounded-lg border bg-card p-5 md:p-6">
        <div className="flex flex-col gap-8 md:flex-row md:items-center">
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-3 md:flex md:w-44 md:flex-col md:gap-2.5 md:border-l md:border-border md:pl-6">
            {telltales.map((t) => <Telltale key={t.label} {...t} />)}
          </div>
        </div>
      </section>
...
function VerdictBanner({ verdict }: { verdict: Verdict }) {
  ...
  const dotClass = verdict.tone === 'neutral' ? 'bg-muted-foreground/50'
    : verdict.tone === 'nominal' ? DOT_BG.nominal : DOT_BG[verdict.tone];
  const tint = live
  ...
  return (
    <div className="flex items-center gap-3 rounded-lg border px-4 py-3" style={tint}>
      ...
      <Text variant="label" tone="default" className="text-[0.7rem]">{verdict.headline}</Text>
      {verdict.more > 0 && (
        <span className="font-mono text-xs tabular-nums text-muted-foreground">+{verdict.more} more</span>
      )}
    </div>
  );
}

interface TelltaleProps { label: string; tone: Tone; count?: number; lit: boolean }

function Telltale({ label, tone, count, lit }: TelltaleProps) {
  return (
    <div className="flex items-center gap-2">
      <span
        className={cn('size-2 shrink-0 rounded-full transition-colors', lit ? DOT_BG[tone] : 'bg-muted-foreground/25')}
        style={lit ? { boxShadow: `0 0 6px 0 var(--color-instrument-${tone})` } : undefined}
        aria-hidden
      />
      <Text as="span" variant="label" tone={lit ? 'default' : 'muted'} className="text-[0.62rem]">{label}</Text>
      {count != null && (
        <span className={cn('ml-auto font-mono text-xs tabular-nums', lit ? 'text-foreground' : 'text-muted-foreground/50')}>
          {count}
        </span>
      )}
    </div>
...
function Cluster({ label, to, linkLabel, children }: {
  label: string; to: '/operator/fleet' | '/operator/storage'; linkLabel: string; children: React.ReactNode;
}) {
  return (
    <section className="space-y-2">
      <div className="flex items-baseline justify-between">
        <Text as="h2" variant="label" tone="muted">{label}</Text>
        <Link to={to}
