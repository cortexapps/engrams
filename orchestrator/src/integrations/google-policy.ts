/** Curated Google API request policy compilation (ADR 0107). */
  ...
  for (const { grant, connection } of resolved) {
    ...
    for (const host of hosts) {
      ...
      const entry: IntegrationInjectJson = {
        ...
        header_template: "",
        secret_ref: "",
        mint_provider: `gcp|${connection.id}|${grant.operation}|${host}`,
      ...
      const conflict = policy.injects.find((candidate) =>
        ...
        candidate.mint_provider.startsWith("gcp|") &&
        candidate.mint_provider !== entry.mint_provider &&
