...
export function makeSpecBlockIterationRoute(deps: SpecBlockIterationRouteDeps): Hono {
  ...
  app.post("/api/v1/specs/:specId/blocks/:blockId/messages", async (c) => {
    ...
    if (!blockId || new TextEncoder().encode(blockId).byteLength > MAX_BLOCK_ID_BYTES) {
...
export function scopedBlockPrompt(input: {
  ...
  return [
    `Work on spec ${input.specId}, block ${input.blockId} in section ${input.sectionId}.`,
    "Read the live section with spec_read before you change it.",
    "If the request needs a document change, use spec_update_block with the exact section_id and block_id above.",
