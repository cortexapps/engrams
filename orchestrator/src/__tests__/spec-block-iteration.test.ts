...
describe("spec block iteration route", () => {
  test("pins a prompt to one validated block and prepares the agent projection", async () => {
    ...
    expect(prompts[0]!.text).toContain("section design");
    expect(prompts[0]!.text).toContain("block request-flow");
    expect(prompts[0]!.text).toContain("Add the retry path.");
  });
  ...
  test("keeps user text inside the scoped request boundary", () => {
