...
| `ENGRAM_GUEST_GATEWAY_PORT`   | Mandatory TCP listener for session-scoped host services (default `13338`). Must be non-zero and match the guest gateway REDIRECT target. |
...
| `ENGRAM_GUEST_GATEWAY_PORT`         | Mandatory guest gateway port (default `13338`; must be non-zero)      |
