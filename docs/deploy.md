...
- **Web + orchestrator** — the same release. The orchestrator
  (Bun/Hono, ADR 0051) owns human auth (better-auth; the login wall
  is the deployment's auth door — ADR 0118) and fronts the
...
- **FC host fleet** — a privileged DaemonSet (`hostNetwork`,
  ...
  these pods; the **operator** does, drain-gated and node-by-node
  (ADR 0044 K3). Rolls REATTACH to running VMs (the FC processes
  escape the pod cgroup); they do not evacuate sessions.
- **The operator** (same fleet release) also owns autoscaling
  (ADR 0048): the coordinator's PG-derived demand signal drives
...
- **Egress proxy** — per node (ADR 0006/0121): tcp/443 TLS-MITM +
  ...
  their new host.
- **Registration is HTTP** (ADR 0013): host-agents POST register/
...
| `ENGRAM_KEK_PROVIDER` | `env-var` \| `gcp-kms` (stub) \| `aws-kms` (real — ADR 0122) |
...
| Variable | Purpose |
|---|---|
| `ENGRAM_COORDINATOR_ENDPOINT` | `http://<coord ClusterIP Service>:8080` — registration + heartbeats are HTTP POSTs (ADR 0013) |
...
| `ENGRAM_NBD_MAX_SLOTS` / `ENGRAM_NBD_WARM_SLOTS` | ADR 0049 slot allocator (module loaded by node-prep with `nbds_max`) |
| `ENGRAM_FC_UFFD_BASE_DIR` (+ ADR 0092 knobs) | The memory substrate tmpfs |
| `ENGRAM_FC_VM_CGROUP_PARENT` | ADR 0044 K2: VMs escape the pod cgroup so rolls don't kill them |
...
- **`aws`** — AWS Secrets Manager (ADR 0122). Refs
...
udp+tcp/53 (ADR 0010). It is mandatory — a host that cannot load the
...
| `aws-secrets-manager` | **AWS production (ADR 0122).** IRSA is env/file-based and WORKS in hostNetwork pods, so the host-agent reads the two SecretIds (`ENGRAM_EGRESS_CA_AWS_{CERT,KEY}_SECRET`) directly. No relay. |
...
the same allow-list as the TLS SNI peeker (ADR 0010). The one
