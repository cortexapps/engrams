...
invalidate-on-CONNECT, and verify-on-read (the `VerifySeed` probe proves the
backend serves the seeded acked bytes — pre-RECONFIGURE since the 2026-07-21
addendum) is the documented fallback, riding P7. The None-arm
...
### Addendum (2026-07-21): the verify-on-read probe races the live guest

Residual-risk item 1 fired, from inside a guard. Four consecutive host-agent
rolls on 2026-07-20/21 parked every survivor whose predecessor left a
non-empty shutdown spool: the P7 verify-on-read probe ran AFTER the
RECONFIGURE, but the kernel releases the guest's parked I/O the instant it
adopts the fresh socket — so the probe raced the resumed guest's own writes.
The probed chunk (the first spooled one) is by construction among the guest's
hottest, so the race lost essentially always in prod (a guest busy enough to
leave a spool is busy enough to rewrite that chunk within the
BLKFLSBUF-widened window), while staying invisible in CI (the FC-lane test
had no concurrent writer). The probe read the guest's fresh write, mis-read
it as a rolled-back base, and the park → quarantine → evict-budget
exhaustion → destroy ladder rewound the very acked writes the spool had
preserved (50–134 MiB per session that night).

Fix: the probe is a dedicated `ReattachStep::VerifySeed` in the
`plan_reattach` output, strictly between `SeedDirtyTier` and `Reconfigure` —
race-free by construction (the guest is still parked), and a failure now
parks the survivor with the kernel config untouched (still dead-parked, so a
later rehydrate attempt can recover the device instead of needing the
evict_local ladder). What the probe proves narrows honestly: pre-RECONFIGURE
it verifies the adoption landed in the backend about to be served (the #810
dropped-seed class); post-RECONFIGURE content equality is *unverifiable in
principle* under a live guest — newest-wins means the guest may legitimately
supersede the seed at any moment after the socket lands. The device-plane
property stays pinned by `nbd_verify_on_read` (no-writer variant), and the
incident shape is pinned by its `reattach_survives_guest_write_racing_reconfigure`
regression (a parked O_DIRECT write to the probed chunk released by the
RECONFIGURE must not fail the rehydrate).
