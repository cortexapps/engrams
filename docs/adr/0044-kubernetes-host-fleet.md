...
| **K2** | VM-detach — a host-agent restart leaves the node's microVMs running; the successor pidfd-reattaches them. Includes the stable-HostId completion | 3 | in review (#123) |
...
**Validation:** macOS compile + clippy (`-D warnings`) + the unit suites (incl. host-id + node-cgroup helpers) are green. `tests/reattach_detach.rs` (create → drop backend → assert FC alive → assert the harness listener is *gone* → reattach → assert it's *re-bound* → destroy-by-pid) is wired into CI's `test-firecracker` job. **Pending — dev-vm, the real survival proof:** on a GKE-like (systemd) cgroup driver, kill the pod's container cgroup scope and assert (a) FC survives (the cgroup escape), (b) the successor reattaches it under the same HostId, (c) the harness reconnects and a prompt round-trips. A single-process cargo test can't simulate a pod cgroup teardown, so this end-to-end loop is the gate before relying on detach in prod — plus a real warm-resumed UFFD/netns VM through the same loop.
