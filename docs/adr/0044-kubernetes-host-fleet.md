...
**Implemented** (cloud-agnostic core): the coordinator exposes the demand signal — Prometheus `engram_session_placement_total{outcome}` (the `no_capacity` pressure counter at `pick_for_session`), `engram_fleet_free_mib`, `engram_fleet_schedulable_hosts` — plus a structured `GET /api/admin/fleet/demand` (schedulable hosts + free/total guest-RAM reservation; single-replica coord, so the in-memory registry is the whole fleet). The K3 operator gains a node-pool autoscaler: a `NodePoolScaler` trait (mirroring engram's `engram-cloud-*` split — policy is OSS, actuator is per-cloud), a `NoopScaler` default that only *logs* the decision, and a unit-tested pure policy `desired_hosts` that grows the pool until free headroom ≥ `targetFreeMib`, clamped to `[minHosts, maxHosts]`. The CRD's `autoscaling` block drives it; with the noop scaler the operator computes + logs "desired N hosts" but doesn't actuate.

**Future follow-ups (explicitly deferred):**
- **GKE actuator** — the `NodePoolScaler` impl in `engram-cloud-gcp` that resizes the real node pool (container API `nodePools.setSize`). Needs a live pool to validate, so it lands separately and flips the operator from observe to actuate.
- **Drain-gated scale-*down*** — v1 is **scale-up only**; the policy never shrinks the pool. Cluster-autoscaler can't be trusted to remove host nodes (it *ignores DaemonSet pods* when deciding, so it would delete a node with **live VMs on it**), so safe scale-down must be operator-driven: pick the least-loaded host, run the K3 drain (snapshot + warm-restore its sessions onto peers), *then* remove its node. Until that lands, idle hosts persist (safe but not cost-optimal); shed them with a manual drain.
