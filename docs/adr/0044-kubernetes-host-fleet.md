...
**Validation:** macOS compile + clippy (`-D warnings`) + the `engram-sandbox-firecracker`/`live_attach` unit suites are green. A new `tests/reattach_detach.rs` FC integration test (create → drop backend → assert FC alive → reattach → destroy-by-pid) is wired into CI's `test-firecracker` job. **Pending:** dev-vm runtime validation of the end-to-end restart, including a real warm-resumed UFFD/netns VM surviving a host-agent pod restart (the netns reattach branch is compile-checked but not yet runtime-exercised). **Known follow-up for K3:** a non-operator restart (crash / `kubectl delete pod`) must complete the reattach pass within the coordinator's host strike-out tolerance, or reconcile could migrate a session whose VM the successor then re-adopts — the drain-gated operator formalizes this; until then it's a heartbeat-gap consideration.

## Status

Proposed. The model + the K8s-first/reframe-Phase-3 decision are settled, the research is folded in, and the **full substrate-parity spike has passed** (above) — KVM + FC boot + NBD chunked-disk daemon + UFFD restore + tap/netns/iptables all run in a privileged k3s pod. The research complicates the call honestly — no *large-scale* production precedent, Intel-only nested virt — but `forkd` proves the shape, our own spike proves the *whole substrate* in-pod, the coordinator-owns-scheduling nuance neutralizes the peers' main objection, and K8s ubiquity serves the deploy-ease goal. K8s-first now stands as **substantiated**, with feasibility retired; what's left is design + production-hardening, not "does it work". Implementation is underway (see the log above): **K1** (Helm chart) shipped, **K2** (VM-detach) is in review. Remaining before Accepted: K2 dev-vm runtime validation, then the **K3** `HostFleet` CRD + drain-gated operator, node-prep + tightened security (device plugin / tainted pool / PSA), **K4** multi-node + autoscaling, and **K5** cutover + MIG retirement. The hostNetwork-vs-pod-netns choice resolved to **pod-netns** (the spike + K2 reattach both use it).
