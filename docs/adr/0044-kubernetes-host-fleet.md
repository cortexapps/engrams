...
- **Proven in-pod:** `/dev/kvm` access + FC microVM create/run + nested-KVM guest boot + virtio-blk rootfs mount + init. One step past forkd (which verified the controller Pod + API, not a full in-pod FC boot).
- **Still to validate (incremental — device/cap/sysctl additions, not new unknowns):** NBD-backed chunked rootfs in-pod (node `nbd` module + `/dev/nbdN` hostPath), tap networking + the netns decision (`NET_ADMIN` + `/dev/net/tun`), UFFD restore (`vm.unprivileged_userfaultfd` node sysctl). **Next spike step:** run the existing `fc_microvm_boots_with_nbd_chunked_rootfs` substrate test inside the pod for full NBD+tap+UFFD parity.

## Status

Proposed. The model + the K8s-first/reframe-Phase-3 decision are settled, the research is folded in, and the **KVM-in-pod + FC-boot spike has passed** (above). The research complicates the call honestly — no *large-scale* production precedent, Intel-only nested virt — but `forkd` proves the shape, our own spike proves the boot, the coordinator-owns-scheduling nuance neutralizes the peers' main objection, and K8s ubiquity serves the deploy-ease goal, so K8s-first stands as defensible-and-now-substantiated. Remaining before Accepted: the operator/autoscaling/security specifics + the NBD/tap/UFFD substrate-parity spike step. No production code yet — this gates the reframed Phase 3.
