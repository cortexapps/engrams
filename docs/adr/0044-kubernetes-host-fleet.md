...
**Validation:** macOS compile + clippy (`-D warnings`) + the `engram-sandbox-firecracker`/`live_attach`/host-id unit suites are green. A new `tests/reattach_detach.rs` FC integration test (create → drop backend → assert FC alive → reattach → destroy-by-pid) is wired into CI's `test-firecracker` job. **Pending:** dev-vm runtime validation of the end-to-end restart, including a real warm-resumed UFFD/netns VM surviving a host-agent pod restart (the netns reattach branch + cross-process stable-id reattach are compile-checked but not yet runtime-exercised).
