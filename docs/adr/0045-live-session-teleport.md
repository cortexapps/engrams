...
**Status: ◐ port DONE + dev-vm-validated, pipeline wired; S-C1 spike pending — the irreversible gate is all-but-pulled.** #139 landed the maintenance surface (the `node-assets-fetch.sh` `ENGRAM_FC_SRC` consume-seam, the `fc_fork` detect lane, the daily `rebase-fc-fork.yml` cron). The fork itself now exists: `cortexapps/firecracker` branch `engram/live-migration` = one clean-room commit on `v1.10.1`, built + unit-tested as a static-musl drop-in on the dev-vm (`firecracker --version` → v1.10.1; memory + api_server snapshot units green). Vendored at `third_party/firecracker`; the `build-firecracker` job (fc_fork-gated, `oras` artifact keyed by fork SHA + the R2 diff-guard) feeds `publish-node-assets` via `ENGRAM_FC_SRC`. Pending: the `stock_fork_snapshot_compat` test + full dev-vm FC-suite proof (M3), the automated-tracking cron upgrade (M4), and the S-C1 go/no-go spike (greenlights Phase C).
