...
**Status: ✅ the fork gate is PULLED — port + pipeline + drop-in proof all done; M4 tracking-cron + the S-C1 spike (Phase C greenlight) remain.** #139 landed the maintenance surface (the `node-assets-fetch.sh` `ENGRAM_FC_SRC` consume-seam, the `fc_fork` detect lane, the daily `rebase-fc-fork.yml` cron). The fork exists: `cortexapps/firecracker` branch `engram/live-migration` = one clean-room commit on `v1.10.1`. Vendored at `third_party/firecracker`; the `build-firecracker` job (fc_fork-gated, `oras` artifact keyed by fork SHA + the R2 diff-guard) feeds `publish-node-assets` via `ENGRAM_FC_SRC` — green end-to-end on real CI.
