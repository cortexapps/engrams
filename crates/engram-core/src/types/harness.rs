...
//! describing its credential contract, native model options, and supported
//! model-router protocols.
...
pub struct HarnessDescriptor {
    ...
    /// Model-router protocols this harness can adapt to. An empty list means
    /// that the harness supports direct/native models only.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub router_protocols: Vec<String>,

    /// Egress the harness needs for every launch, such as telemetry.
    #[serde(default, skip_serializing_if = "HarnessEgress::is_empty")]
    pub egress: HarnessEgress,

    /// Native-provider egress. Direct launches merge this block. Routed
    /// launches replace it with the router's egress.
    #[serde(default, skip_serializing_if = "HarnessEgress::is_empty")]
    pub native_egress: HarnessEgress,
}
...
/// One model or effort option. `env` contains configuration only. Credentials
/// belong to the harness auth contract or a first-class model router.
#[derive(Clone, Debug, Default, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
...
impl HarnessDescriptor {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        for protocol in &self.router_protocols {
            if !matches!(protocol.as_str(), "anthropic_messages" | "openai_responses") {
                return Err(format!(
                    "harness.toml: unsupported router protocol {protocol:?}"
                ));
            }
            if !protocols.insert(protocol) {
                return Err(format!(
                    "harness.toml: duplicate router protocol {protocol:?}"
                ));
            }
        }
        Ok(())
    ...
    for o in opts {
        ...
        for (k, v) in &o.env {
            ...
            if v.contains("{secrets.") {
                return Err(format!(
                    "harness.toml [[{field}]] {:?} env {k:?}: secret references are not allowed in model or effort options",
                    o.id
                ));
            }
...
"#;
        let err = HarnessDescriptor::parse(src).unwrap_err();
        assert!(err.contains("unknown field `secrets`"), "{err}");
    }
...
"#;
    ...
    #[test]
    fn parses_router_protocols_and_separates_native_egress() {
        let descriptor = HarnessDescriptor::parse(
            r#"
name = "codex"
...
"#,
        ...
        .unwrap();
        assert_eq!(descriptor.router_protocols, ["openai_responses"]);
        assert_eq!(descriptor.egress.allow_hosts, ["telemetry.example.com"]);
        assert_eq!(descriptor.native_egress.allow_hosts, ["api.openai.com"]);
    }

    #[test]
    fn rejects_unknown_or_duplicate_router_protocols() {
        for source in [
            "name = \"x\"\nrouter_protocols = [\"chat_completions\"]",
            "name = \"x\"\nrouter_protocols = [\"openai_responses\", \"openai_responses\"]",
        ] {
            assert!(HarnessDescriptor::parse(source).is_err());
        }
    }
}
