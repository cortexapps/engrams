...
impl<D: DockerRunner, P: Ext4Packer> Builder<D, P> {
    ...
    async fn build_after_container(
        ...
        // ADR 0023: an image with a [git] binding gets the forge-seam
        // client glue (askpass + engram-pr + git config + the
        // create-pull-request skill) injected by the platform — thin
        // clients over the baked `engram-agentd forge-*`, so images don't
        // restate this glue in their Dockerfiles.
        if let Some(git) = effective_manifest.git.as_ref() {
...
// ── ADR 0023 forge-seam client glue ────────────────────────────────
// These are thin clients over the baked `engram-agentd forge-*`
// subcommands (the vsock bridge to the coordinator's GitForge lives in
// agentd, which is already injected). Embedded here — not hand-COPY'd in
// each image's Dockerfile — so every `[git]` image gets the capability
// from the platform, versioned with the protocol it calls.

/// `GIT_ASKPASS` helper: returns the fixed `x-access-token` username and,
/// for the password, asks the host for a fresh short-lived credential.
const FORGE_GIT_ASKPASS: &str = r#"#!/bin/sh
# ADR 0023 GIT_ASKPASS helper (injected by the engram image baker).
#
...
/// Provider-agnostic change-request opener.
const FORGE_ENGRAM_PR: &str = r#"#!/bin/sh
# ADR 0023 change-request opener (injected by the engram image baker).
# Thin wrapper over the baked `engram-agentd forge-pull-request`
# subcommand, which dials the host's forge vsock bridge -> coordinator
# GitForge. Flags: --repo owner/name --head <branch> [--base main]
# --title <t> [--body <b>] [--draft]. Prints the created PR/MR URL.
exec engram-agentd forge-pull-request "$@"
"#;

/// System git config wiring the askpass (GitHub provider).
const FORGE_GITCONFIG_GITHUB: &str = r#"# Injected by the engram image baker for images with a [git]
# provider = "github" binding (ADR 0023). git consults the askpass for the
# password; the username is the GitHub App x-access-token convention; the
# empty credential.helper forces the askpass on every operation.
[core]
    askPass = /opt/engram/git-askpass
...
/// Built-in `create-pull-request` skill (teaches the harness to use
/// `engram-pr`). Rides the rootfs per ADR 0023 — built-in skills are
/// baked; the dynamic skills-mount engine is deferred.
const FORGE_CREATE_PR_SKILL: &str = r#"---
...
/// git config wiring them, and the built-in `create-pull-request` skill.
/// GitHub-shaped today (the only `GitForge` impl); other providers are a
/// no-op until their helpers land. The skill is planted at the
/// harness-agnostic `~/.agents/skills` with Claude Code's loader dir
/// symlinked at it.
async fn inject_forge_helpers(
    rootfs_dir: &Path,
    git: &engram_core::types::image::GitConfig,
) -> Result<(), BuildError> {
    if git.provider != engram_core::traits::ForgeKind::GitHub {
        return Ok(());
    }
    write_rootfs_executable(rootfs_dir.join("opt/engram/git-askpass"), FORGE_GIT_ASKPASS).await?;
    write_rootfs_executable(rootfs_dir.join("usr/local/bin/engram-pr"), FORGE_ENGRAM_PR).await?;
    write_rootfs_file(rootfs_dir.join("etc/gitconfig"), FORGE_GITCONFIG_GITHUB).await?;
    write_rootfs_file(
        rootfs_dir.join("root/.agents/skills/create-pull-request/SKILL.md"),
        FORGE_CREATE_PR_SKILL,
    )
    .await?;
...
/// Write `contents` to `path` in the rootfs, creating parent dirs.
async fn write_rootfs_file(path: PathBuf, contents: &str) -> Result<(), BuildError> {
...
/// Like [`write_rootfs_file`] but chmod 0755 (for injected scripts).
async fn write_rootfs_executable(path: PathBuf, contents: &str) -> Result<(), BuildError> {
    write_rootfs_file(path.clone(), contents).await?;
    chmod_executable(&path).await
}

/// Point `/root/.claude/skills` at the harness-agnostic
/// `/root/.agents/skills` so the Claude Code built-in harness discovers
/// baked skills. Idempotent — a no-op if the path already exists (e.g. the
/// image set it up). The symlink target is the guest-absolute path.
async fn ensure_claude_skills_symlink(rootfs_dir: &Path) -> Result<(), BuildError> {
    let claude_dir = rootfs_dir.join("root/.claude");
    tokio::fs::create_dir_all(&claude_dir).await?;
    let link = claude_dir.join("skills");
    if tokio::fs::symlink_metadata(&link).await.is_ok() {
        return Ok(());
    }
    tokio::fs::symlink("/root/.agents/skills", &link)
        .await
        .map_err(|e| BuildError::Config(format!("symlink /root/.claude/skills: {e}")))?;
    Ok(())
}
