    ...
    async fn run_connector_test(
        ...
        let outcome: Result<String, String> = async {
            ...
            let request = if spec.source == "mint" {
                ...
                // hint.host is the integration's OWN served host (e.g. github.com,
                // the git host a multi-host provider validates against) — a
                // different namespace from the connector's egress/API host we GET
                // below (api.github.com). The real mint paths pass the git host
                // (forge.rs) or None (egress broker), never the API host, so do
                // NOT derive it from spec.host or the provider rejects the request
                // ("does not serve host `api.github.com`"). Mint the default
                // credential; the probe still targets spec.host via `url`.
