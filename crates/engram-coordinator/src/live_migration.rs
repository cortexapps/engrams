...
mod tests {
    ...
    /// Issue #208 regression: a client disconnect mid-move must NOT
    /// abandon the verb. We drive the real `teleport_session` axum
    /// handler against a destination whose `migration_capture_postcopy`
    /// blocks on a gate (standing in for the multi-second blackout),
    /// then DROP the handler future while it is parked in capture —
    /// exactly what axum does when the HTTP client goes away.
    ///
    /// Pre-fix the handler awaited `migrate_session_live` INLINE on its
    /// own future, so dropping the handler dropped the verb mid-capture:
    /// the source's `migration_abort` arm never ran (abort_called stays
    /// false) and the lease leaked, opening the split-brain window. With
    /// the fix the verb runs on its own spawned task, so it still
    /// reaches its terminal abort arm (abort_called true) and the
    /// session walks back to Active despite the handler being gone.
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn handler_drop_mid_capture_does_not_abandon_the_verb() {
        use engram_core::types::snapshot::SnapshotMetadata;
        use std::sync::atomic::{AtomicBool, Ordering};
        use std::sync::Arc;

        // The verb must actually take the live path for this test to
        // mean anything. Tests run in their own nextest process (and the
        // local `cargo test <name>` runs this test alone), so setting the
        // gate here does not race a sibling.
        std::env::set_var("ENGRAM_LIVE_TELEPORT", "1");
        assert!(live_teleport_enabled());

        // Capture gate: the handler reaches `migration_capture_postcopy`
        // and parks; the test releases it only AFTER dropping the
        // handler future, reproducing a disconnect mid-blackout.
        let capture_entered = Arc::new(tokio::sync::Notify::new());
        let release_capture = Arc::new(tokio::sync::Notify::new());
        let abort_called = Arc::new(AtomicBool::new(false));

        struct GatedDest {
            capture_entered: Arc<tokio::sync::Notify>,
            release_capture: Arc<tokio::sync::Notify>,
            abort_called: Arc<AtomicBool>,
        }
        #[async_trait::async_trait]
        impl engram_core::traits::SandboxBackend for GatedDest {
            async fn create(
                &self,
                _: engram_core::types::sandbox::SandboxSpec,
            ) -> Result<SandboxId, engram_core::SandboxError> {
            ...
            {
                // THE BLACKOUT. Signal we are here, then block until the
                // test releases us — by then the handler future is gone.
                self.capture_entered.notify_one();
                self.release_capture.notified().await;
                Ok(engram_core::types::snapshot::PostCopyCaptureOut {
                    sealed_chunks: 3,
                    total_chunks: 16,
                    pause_ms: 1,
            ...
            async fn restore(
                ...
                // Dest provably never ran the shipped state ⇒ the verb's
                // zero-loss abort-to-source arm (un-pause + back to Active).
                Err(engram_core::SandboxError::Snapshot(
                    "postcopy-never-loaded: injected dest failure".into(),
                ))
            }
        ...
        let gated: Arc<dyn engram_core::traits::SandboxBackend> = Arc::new(GatedDest {
            capture_entered: capture_entered.clone(),
            release_capture: release_capture.clone(),
            abort_called: abort_called.clone(),
        });
        state.host_registry.register(
            target,
            Arc::new(engram_host_agent::host_client::LocalHostClient::with_noop_hub(gated.clone())),
        );
        // The source backend (capture + abort) is the same gated handle.
        state
            .host_registry
            .record_sandbox_owner(meta.session.lock().sandbox_id.unwrap(), target);
        meta.hosts
        ...
        // Drive the REAL axum handler in a wrapper future we own (boxed
        // so dropping it drops the actual handler future, the way axum
        // drops a handler future when its client disconnects).
        let mut handler = Box::pin(crate::api::admin::teleport_session(
            axum::extract::State(state.clone()),
            axum::extract::Path(session_id),
            axum::Json(crate::api::admin::TeleportSessionRequest {
                target_host_id: target,
            }),
        ));

        // Poll the handler until the verb parks in capture, then DROP it
        // (== client disconnect: axum drops the handler future).
        tokio::select! {
            _ = &mut handler => panic!("handler returned before capture; gate misconfigured"),
            _ = capture_entered.notified() => {}
        }
        drop(handler);

        // The handler is gone. Release capture; on the FIXED code the
        // spawned verb is still alive and proceeds to its abort arm.
        release_capture.notify_one();

        // Wait for the verb to reach its terminal abort arm. On the buggy
        // (inline-await) code the verb was aborted when we dropped the
        // handler, so this never becomes true and we time out → fail.
        let mut aborted = false;
        for _ in 0..200 {
            if abort_called.load(Ordering::SeqCst) {
                aborted = true;
                break;
            }
            tokio::time::sleep(std::time::Duration::from_millis(25)).await;
        ...
        assert!(
            aborted,
            "issue #208: the verb must run to its abort arm despite the \
             handler future being dropped mid-capture (was aborted inline)",
        );

        // And the session walks back to Active (zero loss) — not left
        // stranded mid-move for the evac-resumer to fork.
        let mut walked_back = false;
        for _ in 0..200 {
            if meta.get_session(session_id).await.unwrap().status == SessionState::Active {
                walked_back = true;
                break;
            }
            tokio::time::sleep(std::time::Duration::from_millis(25)).await;
        ...
        assert!(
            walked_back,
            "issue #208: the verb's terminal arm must walk the session \
             back to Active, leaving exactly one live copy",
        );
    }
}
