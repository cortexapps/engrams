...
async fn enabled_image_disk_manifest_round_trips_and_surfaces_in_pin_set() {
    ...
    // ADR 0021 P2: a distinct ref for the base snapshot's disk manifest, so
    // the assertion below can't pass by coincidence with `disk_manifest`.
    let base_mref = ManifestRef {
        manifest_id: Uuid::new_v4(),
        version: 7,
    };
    ...
    // ADR 0021 P2: the base snapshot's disk manifest must round-trip too — it's
    // what the heartbeat advertises so hosts warm the rootfs working set.
    assert_eq!(
