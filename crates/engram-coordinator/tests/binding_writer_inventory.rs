...
const DECLARED: &[(&str, &str, usize, &str)] = &[
    ...
    ("dead_host", "mark_host_dead_if_lease_expired", 1, "the bulk orphan: every non-terminal on a dead host →HostLost unbound"),
    ("dead_host", "transition_session", 1, "THE consolidated settle_host_lost: HostLost→Idle|Dead (RequireUnbound; ADR 0116 A4 — one settle, three callers)"),
    ...
    ("idle_evictor", "fenced_assign_sandbox", 1, "quarantine reap unbind"),
    ...
    ("idle_evictor", "transition_with_fence", 5, "park ladder + quarantine/unbound →HostLost + admin evict entry"),
