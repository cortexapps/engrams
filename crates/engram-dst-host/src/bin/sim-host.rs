//! The host-internal simulator CLI (ADR 0098 Phase 2, P2).
//!
//! ```text
//! sim-host --seed 42 --steps 400 --profile chaos
//! sim-host --seeds 0..50 --steps 400 --profile chaos   # swarm
//! ```
//!
//! Exit 0 = every seed passed the acked-write durability oracle. On failure:
//! prints the seed, the violation, and the trace tail — the failure
//! artifact. Replay = the same binary with the same `--seed`.
//!
...
//! least one seed fails. The arg contract mirrors `engram-dst`'s `sim`
//! binary exactly.
...
fn main() {
    ...
    rt.block_on(async {
        ...
        for s in range.0..range.1 {
            let mut sim = Sim::new(s, profile).await;
            match sim.run(steps).await {
                Ok(report) => {
                    println!(
                        "seed {s}: OK ({} steps, {} writes acked)",
                        report.steps_run, report.writes_acked
                    );
                }
                Err(msg) => {
                    failures += 1;
                    ...
                    eprintln!(
                        "replay: cargo run -p engram-dst-host --release --bin sim-host -- --seed {s} --steps {steps} --profile {profile_name}",
                    );
                    ...
                    let _ = write!(
                         ...
                         **Violation:** `{msg}`\n\n\
                         **Replay:**\n```sh\njust sim-host {s} {steps} {profile_name}\n```\n\n\
