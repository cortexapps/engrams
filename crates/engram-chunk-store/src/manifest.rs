...
/// Stable, versioned identifier for a manifest. The `manifest_id`
/// is permanent (every snapshot of session X uses the same id); the
/// `version` ticks monotonically as the manifest mutates.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash, Serialize, Deserialize)]
