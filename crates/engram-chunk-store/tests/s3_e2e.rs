//! End-to-end against a real `BlobStorage` impl (S3 via MinIO).
//! Validates that the chunk store's typed API works correctly against
//! a backend with wire-level pagination, multipart assembly, etc. —
//! not just the local-fs unit-test impl. The S3 twin of `gcs_e2e.rs`.
//!
//! Gated on `ENGRAM_TEST_S3_ENDPOINT` + `ENGRAM_TEST_S3_BUCKET`.
//! No-op without them.
...
use engram_storage_s3::S3BlobStorage;
use tokio::fs;

/// Connect to the MinIO emulator, or `None` when ungated. Threads the
/// test endpoint through the crate's env override and pins static
/// credentials so the SDK chain never probes IMDS in CI.
async fn emulator_blob() -> Option<Arc<dyn BlobStorage>> {
...
async fn chunk_a_64mib_file_through_s3_round_trips() {
    let Some(blob) = emulator_blob().await else {
        return;
    };
...
/// Cross-session dedup: two distinct manifests built from identical
/// bytes share the same chunks in storage. Verifies the
/// content-addressing property end-to-end against a real backend.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn identical_files_share_chunks_in_s3() {
    let Some(blob) = emulator_blob().await else {
        return;
    };
    let store = ChunkStore::new(blob.clone());
    ...
    for entry in &m1.chunks {
        ...
        assert!(exists, "chunk {} should be present in S3", entry.hash);
    }
