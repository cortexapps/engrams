...
# (ADR 0084 dropped the Homebrew e2fsprogs PATH prepend that lived here:
# Homebrew's mke2fs is built --without-libarchive and cannot pack the
# materializer's tar — the flake's e2fsprogs is the one true binary;
# launch tilt from `nix develop`, or pin ENGRAM_MKE2FS in .env.)
...
def host_agent_resource(name, grpc_port, metrics_port, work_dir, nbd_csv, egress_proxy_port, egress_dns_port):
    env = {
        ...
        # fails closed on `Address already in use` (ADR 0083). The host-agent
    ...
    # ADR 0084: materialization needs a libarchive-enabled (tar-input)
    ...
    if fc_colima_profile:
        ...
        # (macOS gets no branch: ADR 0084 dropped the Homebrew e2fsprogs
        # pins — that mke2fs can't pack the materializer's tar. The flake
        # binary arrives via the nix-develop PATH or the ENGRAM_MKE2FS
        # passthrough above.)
...
# Linux/FC default at 5353 for parity with the binary default (the fc-colima
# host-agent binds inside the VM, where 5353 is free), but avoid it for
# Mac-local VZ dev because mDNS/Chrome commonly owns UDP 5353 on Darwin.
_dns_base_default = '5353'
if uname_str.startswith('Darwin') and not fc_colima_profile:
...
if dev_split:
    ...
    if two_hosts:
        ...
        # fails closed on `Address already in use` (ADR 0083).
